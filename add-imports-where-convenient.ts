import { Project, ts, Symbol, SourceFile, SyntaxKind, Identifier, TypeReferenceNode } from "ts-morph";
...
function getNiceName(sf: SourceFile, checkAvailable: (t: string) => boolean) {
	...
	const res = [fname, dirname.split("-")[0] + " " + fname, dirname + " " + fname, fname + "_"]
		.map(camelize)
		.find(checkAvailable);
	if (res) return res;
	let i = 2;
...
function getReferencedSourceFileFromSymbol(symbol: Symbol) {
	...
	if (declarations.length === 0 || declarations[0].getKind() !== SyntaxKind.SourceFile) return undefined;
	...
	for (const sf of sfs) {
		...
		while (madeChanges > 0) {
			...
			for (const impt of imptypes) {
				const referencedModule = getReferencedSourceFileFromSymbol(impt.getSymbolOrThrow());
				if (!referencedModule) {
			...
			const allNames = new Set(sf.getSymbolsInScope(0b1111111111111111111111111111).map((x) => x.getName()));
			...
			while (todos.length > 0) {
				...
				const myName = getNiceName(referencedModule, (m) => !allNames.has(m));
			...
			for (const [name, referencedModule] of nameMap) {
				const importdecl = sf.addImportDeclaration({
					namespaceImport: name,
					moduleSpecifier: beautifyImportPath(sf.getRelativePathAsModuleSpecifierTo(referencedModule)),
				});
				inverseMap.set(referencedModule.getFilePath(), importdecl.getNamespaceImportOrThrow());
			}
			...
			for (const { node, mod } of toReplace) {
				...
				try {
					node.replaceWithText(
						id.getText() + node.getText().substr(("import()" + node.getArgument().getText()).length)
					);
					madeChanges++;
				} catch (e) {
